...
import { UpdateTipoInstitucion } from "./UpdateTipoInstitucion";
import { apiDeleteTipoInstitucion, apiTipoInstitucionView } from "../api/apiTipoInstitucion";




export const ListaTipoInstitucion = () => {
  ...
  const viewTipo = async () => {
    const tipoList = await apiTipoInstitucionView();
  ...
  const eliminar = async (id) => {
    ...
    if (confirmacion.isConfirmed) {
      let result = await apiDeleteTipoInstitucion(id);
      if (result) {
  ...
  return (
    <>
      <div style={{ marginLeft: "1%" }}>
        ...
        >
          <Link to="/agregarTipoInstitucion" className="nav-link text-center" aria-current="page">
            <button
              className="btnAdd btn-success"
              type="button"
              onClick={(event) => {
                event.preventDefault();
                navigate("/agregarTipoInstitucion");
              }}
            >
