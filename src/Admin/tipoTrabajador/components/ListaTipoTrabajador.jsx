...
import { apiDeleteTipoTrabajador, apiTipoTrabajadorView } from "../api/apiTipoTrabajadorjs";
import { UpdateTipoTrabajador } from "./UpdateTipoTrabajador";



export const ListatipoTrabajador = () => {
  ...
  const viewTipo = async () => {
    const tipoList = await apiTipoTrabajadorView();
  ...
  const eliminar = async (id) => {
    ...
    if (confirmacion.isConfirmed) {
      let result = await apiDeleteTipoTrabajador(id);
      if (result) {
  ...
  return (
    <>
      <div style={{ marginLeft: "1%" }}>
        ...
        >
          <Link to="/agregarTipoTrabajador" className="nav-link text-center" aria-current="page">
            <button
              className="btnAdd btn-success"
              type="button"
              onClick={(event) => {
                event.preventDefault();
                navigate("/agregarTipoTrabajador");
              }}
            >
