...
import { apiDeleteTipoPago, apiTipoPagoView } from "../api/apiTipoPago";
import { UpdateTipoPago } from "./UpdateTipoPago";


export const ListaTipoPago = () => {
  ...
  const viewTipo = async () => {
    const tipoList = await apiTipoPagoView();
  ...
  const eliminar = async (id) => {
    ...
    if (confirmacion.isConfirmed) {
      let result = await apiDeleteTipoPago(id);
      if (result) {
  ...
  return (
    <>
      <div style={{ marginLeft: "1%" }}>
        ...
        >
          <Link to="/agregarTipoPago" className="nav-link text-center" aria-current="page">
            <button
              className="btnAdd btn-success"
              type="button"
              onClick={(event) => {
                event.preventDefault();
                navigate("/agregarTipoPago");
              }}
            >
